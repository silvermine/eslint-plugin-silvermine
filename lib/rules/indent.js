 ...
 * @deprecated
 *
 * THIS RULE IS NOT MAINTAINED, AND SHOULD NOT BE USED. Our config project has switched
 * back to using the default eslint rule because the bugs we found in it were eventually
 * fixed. In 3.0.0 of the plugin, we will delete this rule.
 *
 * Ironically, this rule no longer passes the indentation rules we define in our config,
 * and fixing it is not worth the hassle, so we are also disabling the indentation
...
* With the following changes applied: https://github.com/eslint/eslint/compare/83017a3..9738f8c
...
module.exports = {
   ...
   create(context) {
      ...
      function report(token, neededIndent) {
         ...
         const numSpaces = actualIndent.filter((char) => { return char === ' '; }).length;

         const numTabs = actualIndent.filter((char) => { return char === '\t'; }).length;
      ...
      const baseOffsetListeners = {
         ...
         AssignmentExpression(node) {
            const operator = sourceCode.getFirstTokenBetween(node.left, node.right, (token) => { return token.value === node.operator; });
         ...
         'DoWhileStatement, WhileStatement, ForInStatement, ForOfStatement': (node) => { return addBlocklessNodeIndent(node.body); },
         ...
         ImportDeclaration(node) {
            if (node.specifiers.some((specifier) => { return specifier.type === 'ImportSpecifier'; })) {
               const openingCurly = sourceCode.getFirstToken(node, astUtils.isOpeningBraceToken),
                     closingCurly = sourceCode.getLastToken(node, astUtils.isClosingBraceToken),
                     nodeSpecifierFilter = (specifier) => { return specifier.type === 'ImportSpecifier'; };

               addElementListIndent(node.specifiers.filter(nodeSpecifierFilter), openingCurly, closingCurly, options.ImportDeclaration);
            }

            const fromToken = sourceCode.getLastToken(node, (token) => { return token.type === 'Identifier' && token.value === 'from'; });

            const sourceToken = sourceCode.getLastToken(node, (token) => { return token.type === 'String'; });

            const semiToken = sourceCode.getLastToken(node, (token) => { return token.type === 'Punctuator' && token.value === ';'; });
         ...
         SwitchStatement(node) {
            ...
            if (node.cases.length) {
               ...
               tokensBetween.forEach((token) => { return offsets.ignoreToken(token); });
            }
         },
         ...
         VariableDeclaration(node) {
            ...
            variableIndent = Object.prototype.hasOwnProperty.call(options.VariableDeclarator, node.kind)
            ...
            if (node.declarations[node.declarations.length - 1].loc.start.line > node.loc.start.line) {
                ...
                * ];
                */
               offsets.setDesiredOffsets(node.range, firstToken, variableIndent, extraVariableIndent);
            } else {
      ...
      const offsetListeners = lodash.mapValues(
          ...
          */
         (listener) => { return (node) => { return listenerCallQueue.push({ listener, node }); }; }
      );
      ...
      const ignoredNodeListeners = options.ignoredNodes.reduce(
         (listeners, ignoredSelector) => { return Object.assign(listeners, { [ignoredSelector]: addToIgnoredNodes }); },
         {}
      );
            ...
            'Program:exit'() {
               ...
               if (options.ignoreComments) {
                  sourceCode.getAllComments()
                     .forEach((comment) => { return offsets.ignoreToken(comment); });
               }

                // Invoke the queued offset listeners for the nodes that aren't ignored.
               listenerCallQueue
                  .filter((nodeInfo) => { return !ignoredNodes.has(nodeInfo.node); })
                  .forEach((nodeInfo) => { return nodeInfo.listener(nodeInfo.node); });
               ...
               sourceCode.lines.forEach((line, lineIndex) => {
                  ...
                  if (astUtils.isCommentToken(firstTokenOfLine)) {
                     ...
                     /*
                      * If a comment precedes a line that begins with a semicolon token,
                      * align to that token, i.e.
                      *
                      * let foo
                      * // comment
                      * ;(async () => {})()
